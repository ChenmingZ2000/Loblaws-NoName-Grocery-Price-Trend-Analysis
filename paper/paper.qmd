...
kable(head(sorted_data, 7), 
      col.names = c("product_id", "date", "current_price", "price_per_unit", "product_name", "units", "type", "size"), 
...
Define yiy_i as the current price of the product in the ii-th observation, assumed to follow a Gamma distribution with mean $\mu_i$ and shape parameter $\phi$. This model accounts for variations across different product sizes, types, and temporal trends, with random intercepts for each product ID.

The linear predictor for the mean ($\mu_i$) of the Gamma distribution is defined as follows:
$$
\log(\mu_i) = \alpha + \beta_1 \cdot \text{size}_ i + \beta_2 \cdot \text{type}_i + \beta_3 \cdot \text{date}_i + u_{\text{product_id}[i]} \\
$$

where:
$y_i$ is the observed current price for the ii-th product.
$\mu_i$ is the predicted mean price for the ii-th product, modeled on a log scale.
